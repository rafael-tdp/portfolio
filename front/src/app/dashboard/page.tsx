    ...
    <div className="min-h-screen px-4 py-4 sm:p-6 bg-gray-50">
      <div className="max-w-4xl mx-auto">
            ...
            {user ? (
              <div className="flex items-center gap-3 justify-between">
                <div className="text-sm">
                  <div className="font-medium truncate max-w-[150px] sm:max-w-none">{user.name || user.fullName || user.email}</div>
                  {user.email && <div className="text-xs text-gray-500 truncate max-w-[150px] sm:max-w-none">{user.email}</div>}
                </div>
                <Button
                  onClick={logout}
                  loading={loggingOut}
        ...
        {!loadingStats && visitStats && (
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 mb-6">
            <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 flex items-center gap-3 sm:gap-4">
              ...
              <div>
                <div className="text-xl sm:text-2xl font-bold text-gray-800">{visitStats.totalVisits}</div>
                <div className="text-xs sm:text-sm text-gray-400">Visites totales</div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <div className="text-xs sm:text-sm text-gray-400">Visiteurs uniques</div>
              </div>
            </div>
            <div className="bg-white rounded-xl border border-gray-100 p-4 sm:p-5 flex items-center gap-3 sm:gap-4">
              ...
              <div>
                ...
                <div className="text-xs sm:text-sm text-gray-400">Dernière visite</div>
              </div>
            </div>
          </div>
        )}
        ...
        {!loadingStats && visitStats && visitStats.visits.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 mb-6 overflow-hidden">
            <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-100 bg-gray-50/50">
              <h2 className="font-semibold text-gray-800 text-sm sm:text-base">Visites récentes</h2>
            </div>
              ...
              {visitStats.visits.slice(0, 20).map((v, index) => (
                ...
                >
                  ...
                  <div className="text-right flex-shrink-0 ml-2">
                    <div className="text-xs sm:text-sm text-gray-600">{formatDate(v.createdAt)}</div>
        ...
        {!loadingStats && visitStats && visitStats.applications.length > 0 && (
          <div className="bg-white rounded-xl border border-gray-100 mb-6 overflow-hidden">
            <div className="px-4 sm:px-5 py-3 sm:py-4 border-b border-gray-100 bg-gray-50/50">
              <h2 className="font-semibold text-gray-800 text-sm sm:text-base">Statistiques par candidature</h2>
            </div>
            <div className="max-h-60 overflow-y-auto">
              {visitStats.applications.map((app, index) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-gray-800 truncate text-xs sm:text-sm">{app.jobTitle || "Sans titre"}</div>
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 ml-2 sm:ml-4 flex-shrink-0">
                    <div className="flex flex-col items-center px-2 sm:px-3 py-1 sm:py-1.5 bg-sky-50 rounded-lg min-w-[45px] sm:min-w-[60px]">
                      <div className="text-xs sm:text-sm font-bold text-sky-600">{app.totalVisits}</div>
                      <div className="text-[6px] sm:text-[7px] uppercase tracking-wide text-sky-400 font-medium">visites</div>
                    </div>
                    <div className="flex flex-col items-center px-1 sm:px-1 py-1 bg-emerald-50 rounded-lg min-w-[45px] sm:min-w-[60px]">
                      <div className="text-xs sm:text-sm font-bold text-emerald-600">{app.uniqueVisitors}</div>
                      <div className="text-[6px] sm:text-[7px] uppercase tracking-wide text-emerald-400 font-medium">uniques</div>
                    </div>
                  </div>
                </div>
              ))}
        ...
        {!loadingStats && (!visitStats || visitStats.totalVisits === 0) && (
          <div className="bg-white rounded-xl border border-gray-100 p-6 sm:p-8 mb-6 text-center">
            ...
            <div className="text-gray-600 font-medium text-sm sm:text-base">Aucune visite enregistrée</div>
            <div className="text-xs sm:text-sm text-gray-400 mt-1">Partagez vos pages de candidature pour commencer à recevoir des visites</div>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
          ...
          >
            <div className="font-semibold text-gray-800 text-sm sm:text-base">Mes candidatures</div>
            <div className="text-xs sm:text-sm text-gray-400 mt-1">Voir et gérer vos candidatures</div>
          </Link>
          ...
          <Link
            ...
            className="block p-4 sm:p-5 bg-white rounded-xl border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all"
          >
            <div className="font-semibold text-gray-800 text-sm sm:text-base">Nouvelle candidature</div>
            <div className="text-xs sm:text-sm text-gray-400 mt-1">Créer une nouvelle candidature</div>
          </Link>
        </div>
      </div>
    </div>
